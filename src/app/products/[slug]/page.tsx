...
const Contact = () => {
  ...
  const onFormSubmit = (data: ContactForm) => {
    ...
    fetch(`https://apiraphaeldoucet.onrender.com/products/${id}`, {
      method: 'PUT',
      headers: {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`https://apiraphaeldoucet.onrender.com/products/${id}`);
        const data = await response.json();
        setValue('title', data.title);
        setValue('description', data.description);
        setValue('categoriesId', data.categoriesId);
      } catch (error) {
        console.error(error);
      }
    };
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Grid container spacing={2} justifyContent="flex-start">
                  ...
                  <Grid item>
                    <Button variant="contained" startIcon={<DeleteOutlineIcon />} color="error" onClick={deleteProduct}>
                      DELETE PRODUCT
                    </Button>
                  </Grid>
                </Grid>
